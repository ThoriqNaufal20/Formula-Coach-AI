...
function safeStorageSet(key, value) {
  try {
    ...
    /* storage tidak tersedia, lanjut tanpa menyimpan */
  }
}
...
async function initProviderSelector() {
  const saved = safeStorageGet(PROVIDER_STORAGE_KEY);
...
el.providerSelect.addEventListener("change", () => {
  safeStorageSet(PROVIDER_STORAGE_KEY, el.providerSelect.value);
  fetch("/api/providers")
    .then((r) => r.json())
...
function loadWorkbook() {
  try {
    const raw = safeStorageGet(WORKBOOK_STORAGE_KEY);
    if (raw) {
...
function loadHistory() {
  try {
    const raw = safeStorageGet(HISTORY_STORAGE_KEY);
...
function saveHistory(history) {
  safeStorageSet(HISTORY_STORAGE_KEY, JSON.stringify(history));
}
...
el.sheetGrid.addEventListener("click", (e) => {
  const btn = e.target.closest(".cell-remove-btn");
  if (!btn) return;
  if (btn.dataset.removeRow !== undefined) removeRow(Number(btn.dataset.removeRow));
...
function renderGrid() {
  ...
  for (let c = 0; c < colCount; c++) {
    ...
    if (colCount > MIN_COLS) {
      ...
      removeBtn.dataset.removeCol = c;
      colHead.appendChild(removeBtn);
    }

    fragment.appendChild(colHead);
  }
